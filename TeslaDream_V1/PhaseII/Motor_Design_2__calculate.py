...
class m():
    ...
    def T(self,R,S,P):
        ...
        Sh = AWG_Diameter(Iph)[0]/1000      # WireDensity/Topology * WireWidth = SlotHeight
        Sh = Sh*P['WireDensity']/P['Topology']
        if Sh<AWG_Diameter(Iph)[0]/1000: # Can't physically be less than wirewidth
            Sh=AWG_Diameter(Iph)[0]/1000

        Nslots = (2*R['Ns'])/P['WireDensity'] # (Wire=Turns*2)(Slots/Wire) bc each 1 turn = 2 slotwires
        AWG = AWG_Diameter(Iph)[1]

        return Torque,[I1,I2,R1,P['R2']],[Sw,Sh,Nslots,AWG]
